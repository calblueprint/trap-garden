...
import { updateUserPlantDetails } from '@/api/supabase/queries/userPlants';
...
import { Container, Header, PlantDetailsText } from './style';
      ...
      {isEditing ? (
        ...
        <>
          ...
          <Flex $direction="column" $gap="8px">
            {DetailRow(
              'calendar',
              `Date Planted: ${formatTimestamp(plantedDate)}`,
            )}
            {DetailRow(
              'plantHand',
              `Planting Type: ${toTitleCase(typeOfPlanting)}`,
            )}
            {recentHarvestDate &&
              DetailRow(
                'plant',
